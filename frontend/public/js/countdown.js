...
async function updateCountdown() {
    ...
    const timeDelta = Math.max(nextGame.date - currentTime, 0);

    // console.log(nextGame);
    // console.log(nextGame.date);

    const daysDelta = Math.floor(timeDelta/(24*60*60));
    const hoursDelta = Math.floor(timeDelta/(60*60)) % 24;
    const minutesDelta = Math.floor(timeDelta/(60)) % 60;
    const secondsDelta = Math.floor(timeDelta) % 60;
